...
struct SequenceFiboBuilderFixture : public testing::Test {
  ...
  SequenceFiboBuilderFixture()
	  ...
	  sb (std::make_unique<task::seventh::SequenceBuilder<ullint,
		 task::seventh::Fibo>>(this->low, this->high,
		 	task::seventh::GenRule<ullint,
		 	task::seventh::Fibo>()))
  {}
  ...
  std::unique_ptr<task::seventh::SequenceBuilder<ullint,
												 task::seventh::Fibo>> sb;
};

//---------------------------- CTOR TESTS ------------------------------------

TEST(SequenceFiboBuilderTest, CtorTest) {
  auto sb = std::make_unique<task::seventh::SequenceBuilder<ullint,
	task::seventh::Fibo>>(0ull, 1ull, task::seventh::GenRule<ullint,
		task::seventh::Fibo>());
